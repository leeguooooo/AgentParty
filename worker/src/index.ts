...
app.post("/api/channels", async (c) => {
  ...
  // 守卫属于高级、显式启用能力。新频道默认关闭；需要限制自动对话时由频道管理员开启。
  // workflow_guard_enabled 的数据库默认值同样是 0。
  const insertChannel = (candidate: string) =>
    c.env.DB.prepare(
      "INSERT INTO channels (slug, title, kind, mode, visibility, created_by, owner_account, created_at, loop_guard_enabled, workflow_guard_enabled) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, 0)",
    )
