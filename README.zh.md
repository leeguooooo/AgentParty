...
启动器会先跑一次不调用模型的预检，拒绝打开那种「看着在线、其实没在听」的 Claude session。外壳到底装了
什么、两道 opt-in 分别管什么、活动上报的边界、`party doctor claude-plugin` 与各个验收工具，见
[docs/claude-plugin.zh.md](docs/claude-plugin.zh.md)。
...
这道闸是 fail-closed 的，证据规则很严格——peer 发现、发送屏障、Hook 事件绑定，以及验收工具认与不认
什么，见 [docs/cross-session-internals.zh.md](docs/cross-session-internals.zh.md)。
