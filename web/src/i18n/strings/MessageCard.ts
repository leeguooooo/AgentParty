import { registerDict, type LocaleDict } from "../dict";

export const MessageCardStrings: LocaleDict = {
  en: {
    "MessageCard.badge.edited": "edited",
    "MessageCard.badge.retracted": "retracted",
    "MessageCard.retracted": "message retracted",
    "MessageCard.menu.reply": "reply",
    "MessageCard.menu.edit": "edit",
    "MessageCard.menu.task": "turn into task",
    "MessageCard.menu.retract": "retract",
    "MessageCard.menu.copy": "copy text",
    "MessageCard.menu.more": "message actions",
    "MessageCard.copied": "copied",
    "MessageCard.edit.save": "save",
    "MessageCard.edit.saving": "saving…",
    "MessageCard.edit.cancel": "cancel",
    "MessageCard.reply.retracted": "original message retracted",
    "MessageCard.reply.jump": "jump to original message #{seq}",
    "MessageCard.decision.state.pending": "decision · pending",
    "MessageCard.decision.state.resolved": "decision · resolved",
    "MessageCard.decision.state.auto_resolved": "decision · auto",
    "MessageCard.decision.responseBadge": "decision response",
    "MessageCard.decision.approve": "approve",
    "MessageCard.decision.reject": "reject",
    "MessageCard.decision.awaiting": "awaiting a human decision",
    "MessageCard.decision.resolved": "resolved → {option}",
    "MessageCard.decision.autoResolved": "auto-resolved (unattended) → {option}",
    "MessageCard.clientVersion.title": "sender CLI version v{version}",
    "MessageCard.clientVersion.outdated": "sender CLI v{version} is behind the recommended v{min} — upgrade suggested",
    "MessageCard.receptionReply": "{runner} reception reply",
    "MessageCard.receptionReply.started":
      "Generated by a separate per-channel reception session; it does not inherit the owner's open conversation. This session started for trigger #{seq}.",
    "MessageCard.receptionReply.resumed":
      "Generated by a separate per-channel reception session; it does not inherit the owner's open conversation. The reception session resumed for trigger #{seq}.",
    "MessageCard.receptionReply.unavailable":
      "Generated by the resident reception runner for trigger #{seq}; no resumable session handle was available.",
    // 回执（#828）：徽标文案。它表达的是「收到了」，绝不能读成「处理完了」——手搓版当年就是被读成了
    // 本人表态，所以 title 里明写 not a reply。
    "MessageCard.receipt": "received by {names}",
    "MessageCard.receipt.reason.not_in_turn": "{name} received this but is not in a turn right now",
    "MessageCard.receipt.reason.queued": "{name} has this queued and is busy",
    "MessageCard.receipt.reason.seen": "{name} saw this",
    "MessageCard.receipt.hint": "Receipt only — not a reply, and not a sign the work is done.",
    "MessageCard.owner.unresolved": "Owner unknown",
    "MessageCard.agent.ownedLabel": "{owner} · {name}",
    "MessageCard.agentCard.identity": "Identity",
    "MessageCard.agentCard.owner": "Owned by",
    "MessageCard.agentCard.technicalId": "Technical ID",
    "MessageCard.agentCard.leader": "Leader",
    "MessageCard.agentCard.division": "Role & division",
    "MessageCard.agentCard.current": "Current work",
    "MessageCard.agentCard.recent": "Recent work",
    "MessageCard.agentCard.none": "Not reported",
    "MessageCard.agentCard.noRecent": "No recent work in the loaded channel history",
  },
  zh: {
    "MessageCard.badge.edited": "已编辑",
    "MessageCard.badge.retracted": "已撤回",
    "MessageCard.retracted": "消息已撤回",
    "MessageCard.menu.reply": "引用回复",
    "MessageCard.menu.edit": "编辑",
    "MessageCard.menu.task": "转为任务",
    "MessageCard.menu.retract": "撤回",
    "MessageCard.menu.copy": "复制文本",
    "MessageCard.menu.more": "消息操作",
    "MessageCard.copied": "已复制",
    "MessageCard.edit.save": "保存",
    "MessageCard.edit.saving": "保存中…",
    "MessageCard.edit.cancel": "取消",
    "MessageCard.reply.retracted": "原消息已撤回",
    "MessageCard.reply.jump": "跳转到原消息 #{seq}",
    "MessageCard.decision.state.pending": "决策 · 待定",
    "MessageCard.decision.state.resolved": "决策 · 已决",
    "MessageCard.decision.state.auto_resolved": "决策 · 自动",
    "MessageCard.decision.responseBadge": "决策回应",
    "MessageCard.decision.approve": "通过",
    "MessageCard.decision.reject": "拒绝",
    "MessageCard.decision.awaiting": "等待人类决策",
    "MessageCard.decision.resolved": "已决 → {option}",
    "MessageCard.decision.autoResolved": "自动放行（无人值守）→ {option}",
    "MessageCard.clientVersion.title": "发送方 CLI 版本 v{version}",
    "MessageCard.clientVersion.outdated": "发送方 CLI v{version} 落后于推荐版本 v{min}，建议升级",
    "MessageCard.receptionReply": "{runner} 接待回复",
    "MessageCard.receptionReply.started":
      "由独立频道接待会话生成，不继承 owner 当前打开的会话；本接待会话为触发消息 #{seq} 新建。",
    "MessageCard.receptionReply.resumed":
      "由独立频道接待会话生成，不继承 owner 当前打开的会话；本接待会话为触发消息 #{seq} 续接。",
    "MessageCard.receptionReply.unavailable":
      "由常驻接待 runner 为触发消息 #{seq} 生成；当前没有可恢复的会话句柄。",
    "MessageCard.receipt": "{names} 已收到",
    "MessageCard.receipt.reason.not_in_turn": "{name} 收到了，但当前不在轮次里",
    "MessageCard.receipt.reason.queued": "{name} 已排进队列，正忙",
    "MessageCard.receipt.reason.seen": "{name} 看到了",
    "MessageCard.receipt.hint": "仅为回执——不是回复，也不代表已处理。",
    "MessageCard.owner.unresolved": "归属待确认",
    "MessageCard.agent.ownedLabel": "{owner} · {name}",
    "MessageCard.agentCard.identity": "身份",
    "MessageCard.agentCard.owner": "归属用户",
    "MessageCard.agentCard.technicalId": "技术 ID",
    "MessageCard.agentCard.leader": "所属 Leader",
    "MessageCard.agentCard.division": "角色与分工",
    "MessageCard.agentCard.current": "进行中的工作",
    "MessageCard.agentCard.recent": "最近工作",
    "MessageCard.agentCard.none": "未上报",
    "MessageCard.agentCard.noRecent": "当前已加载频道记录里没有最近工作",
  },
};

registerDict(MessageCardStrings);
