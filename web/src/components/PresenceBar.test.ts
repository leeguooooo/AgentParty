...
describe("名单里的踢出（#1070）", () => {
  test("版主可直接踢人或踢 agent；非版主看不到；system 行没有踢出", () => {
    const removed: Array<{ name: string; also?: readonly string[] }> = [];
    const onRemoveParticipant = (name: string, also?: readonly string[]) => { removed.push({ name, also }); };
    const now = Date.now();
    const presence = {
      ...
      next = create(
        createElement(
          ...
          createElement(PresenceBar, {
            ...
            participants: [{ name: "leo", kind: "human", owner: "acct-leo", handle: "leo" }],
            status: "open",
            canModerate: true,
            initialRosterOpen: true,
            onRemoveParticipant: (name: string, also?: readonly string[]) => { removed.push({ name, also }); },
          } as never),
        ),
      );
    });
    renderer = next;
    const kick = next.root.findAll((n) => n.type === "button" && n.props.className === "presence-kick");
    expect(kick).toHaveLength(1);
    act(() => kick[0]!.props.onClick());
    expect(removed[0]!.name).toBe("leo");
    expect([...(removed[0]!.also ?? [])].sort()).toEqual(["leo", "leo-old"]);
  });
});
