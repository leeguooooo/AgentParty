...
// #504 团队面板博客风：两个页签（01 成员 / 02 工作）替代一整页长滚动。
...
function render(): ReactTestRenderer {
  ...
  act(() => {
    r = create(
      <LocaleProvider>
        <TeamTabs
          stats={{ roles: 3, online: 3, offline: 7, unclaimed: 10, pendingClaims: 2 }}
...
describe("TeamTabs (#504 博客风页签)", () => {
  ...
  test("没有未认领成员时仍显示 0 状态角标，但不标红", () => {
    ...
    act(() => {
      r = create(
        <LocaleProvider>
          <TeamTabs
            stats={{ roles: 3, online: 3, offline: 0, unclaimed: 0, pendingClaims: 0 }}
            division={<div>DIVISION_PANEL</div>}
            board={<div>BOARD_PANEL</div>}
          />
        </LocaleProvider>,
  ...
  test("待确认认领数大于 0 时显示独立的高亮状态指标", () => {
    const r = render();
    const stats = r.root.findAllByProps({ role: "listitem" });
  ...
  test("saving a role disables Team close and ignores a direct close request", () => {
    ...
    const close = renderer!.root.findByProps({ className: "d-btn team-blog-close" });
    expect(close.props.disabled).toBe(true);
    act(() => close.props.onClick());
    expect(closeCalls).toBe(0);
  });

  test("两个面板保持挂载，默认只显示成员页", () => {
    const r = render();
  ...
  test("两个页签编号 01/02，未认领角标挂在成员页", () => {
    const r = render();
    expect(tabButtons(r).length).toBe(2);
    const text = allText(r);
  ...
  test("点第二个页签切到工作", () => {
    ...
    expect(tabPanels(r).map((panel) => panel.props.hidden)).toEqual([true, false]);
  });
  ...
  test("页签使用 roving tabIndex、ARIA 关联和左右方向键循环切换", () => {
    ...
    expect(tabs.map((button) => button.props.tabIndex)).toEqual([0, -1]);
    expect(new Set(tabs.map((button) => button.props["aria-controls"])).size).toBe(2);
    ...
    expect(tabs.map((button) => button.props.tabIndex)).toEqual([-1, 0]);
    panels = tabPanels(r);
    expect(panels.map((panel) => panel.props.hidden)).toEqual([true, false]);

    act(() => {
      tabs[1]!.props.onKeyDown({ key: "ArrowRight", preventDefault: () => {} });
    });
    tabs = tabButtons(r);
    expect(tabs.map((button) => button.props.tabIndex)).toEqual([0, -1]);
    act(() => {
  ...
  test("切换页签不会重置子模块的本地状态", () => {
    ...
    act(() => {
      renderer = create(
        <LocaleProvider>
          <TeamTabs
            stats={{ roles: 3, online: 3, offline: 7, unclaimed: 10, pendingClaims: 0 }}
            division={<div>DIVISION_PANEL</div>}
            board={<StatefulBoard />}
          />
        </LocaleProvider>,
    ...
    act(() => tabButtons(r)[0]!.props.onClick());

    expect(r.root.findByProps({ "aria-label": "board draft" }).props.value).toBe("keep this draft");
    expect(tabPanels(r).map((panel) => panel.props.hidden)).toEqual([false, true]);
  ...
  test("详情在同一外壳内替代页签，返回后恢复此前选中的页签", () => {
    ...
    expect(tabPanels(r).map((panel) => panel.props.hidden)).toEqual([true, true]);
    ...
    expect(tabButtons(r).map((button) => button.props["aria-selected"])).toEqual([false, true]);
    expect(tabPanels(r).map((panel) => panel.props.hidden)).toEqual([true, false]);
  });
