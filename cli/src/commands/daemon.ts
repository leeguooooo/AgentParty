...
import { agentpartyHome, loadCursor, resolveChannel, saveCursor } from "../config";
...
scope（#692②，别继承交互式全家桶）：SDK session 被收口到 settingSources:[]（不读全局 ~/.claude 设置 / MCP
  ...
  受控 cwd=<AGENTPARTY_HOME 或 ~/.agentparty>/daemon/<channel>（不在用户当前目录里跑）。daemon 只产文本回帖。
...
/**
 * #692②：daemon 的受控 cwd——专用隔离工作目录 ~/.agentparty/daemon/<channel>，不在用户启动 daemon 的当前
 * 目录里跑 SDK。避免 SDK 的 project 探测 / 任何文件操作落到用户工作树（尤其别顺手读到 cwd 里的 CLAUDE.md /
 * 项目文件）。每频道一个，best-effort 建目录。
 */
export function daemonWorkdir(channel: string): string {
  return join(agentpartyHome(), "daemon", channel);
}

/**
 * 把「受控 cwd 是否可用」映射成传给 createSdkRunner 的 overrides：拿到目录就 scope cwd，拿不到就传 {}
 * （SDK 回退默认 cwd；settingSources:[] 仍保证不读全局配置）。抽成纯函数便于测两条分支的接线不回归。
 */
export function daemonRunnerOptions(workdir: string | undefined): Record<string, unknown> {
  return workdir === undefined ? {} : { cwd: workdir };
}
...
export async function run(argv: string[]): Promise<number> {
  ...
  // #692②：受控 cwd。best-effort 建目录；建不出来（权限等）就不设 cwd（SDK 回退默认 cwd，settingSources:[]
  // 仍保证不读全局配置），不阻塞启动。失败时留一条**固定文案**告警（不带路径/错误详情，避免泄漏用户目录），
  // 让操作者知道 cwd 隔离这次没生效。
  let workdir: string | undefined;
  try {
    workdir = daemonWorkdir(channel);
    mkdirSync(workdir, { recursive: true });
  } catch {
    workdir = undefined;
    console.error("party daemon: could not create the isolated daemon workdir; running the SDK without a scoped cwd.");
  }
  ...
  try {
    const code = await runDaemon({
      ...
      runner: createSdkRunner(daemonRunnerOptions(workdir)),
